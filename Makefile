...
TOP_PATH = .
LINUX_SPI_PATH = ./linux_spi
LINUX_GPIO_PATH = ./linux_gpio
ICM20608_PATH = ./icm20608
...
LINUX_SPI_SRC = $(wildcard $(LINUX_SPI_PATH)/*.c)
LINUX_SPI_OBJ = $(patsubst %.c, %.o, $(LINUX_SPI_SRC))

LINUX_GPIO_SRC = $(wildcard $(LINUX_GPIO_PATH)/*.c)
LINUX_GPIO_OBJ = $(patsubst %.c, %.o, $(LINUX_GPIO_SRC))

ICM20608_SRC = $(wildcard $(ICM20608_PATH)/*.c)
ICM20608_OBJ = $(patsubst %.c, %.o, $(ICM20608_SRC))

# 所有目标文件的集合(根据项目情况修改)
TARGET_OBJS := $(TOP_OBJ) $(LINUX_SPI_OBJ) $(LINUX_GPIO_OBJ) $(ICM20608_OBJ)
